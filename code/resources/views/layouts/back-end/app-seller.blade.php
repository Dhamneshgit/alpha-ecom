...
<body class="footer-offset">
    ...
    {{-- <script>
        setInterval(function() {
        $.ajax({
            url: '{{ route('seller.plan.track-plan-status') }}',
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',  
            },
            ...
            success: function(response) {
                ...
                if (response.status === 'inactive') {
                    console.log('Plan is expired, showing SweetAlert');

                    swal({
                        title: 'Plan Expired',
                        text: 'Please purchase a plan to continue using the service.',
                        type: 'warning',  
    ...
    // Redirect function
    function check_plan() {
        console.log('Redirecting to plan list...');
        location.href = '{{ route('seller.plan.plan-list') }}';
    }
    ...
    <script>
        setInterval(function() {
            // Check if the current route is not 'seller.plan.plan-list'
            // console.log(window.location.pathname);
            
            if (window.location.pathname !== '/seller/plan/plan-list') {
                $.ajax({
                    url: '{{ route('seller.plan.track-plan-status') }}',
                    type: 'POST',
                    data: {
