            ...
            <div class="card-body">
                <form id="submit-create-role" action="{{ route('admin.custom-role.update', [$role['id']]) }}" method="post"
                        ...
                        <div class="col-lg-6">
                            <div class="form-group mb-4">
                                ...
                                <select name="name" class="form-control" id="name" required>
                                    ...
                                    <option value="Accountant" @if ($role['name'] == 'Accountant') selected @endif>Accountant</option>
                                    <option value="Representative" @if ($role['name'] == 'Representative') selected @endif>Representative</option>
                                    <option value="Manager" @if ($role['name'] == 'Manager') selected @endif>Manager</option>
                                    <option value="Customer Controller" @if ($role['name'] == 'Customer Controller') selected @endif>Customer Controller</option>
                                    <option value="Vendor Controller" @if ($role['name'] == 'Vendor Controller') selected @endif>Vendor Controller</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3 col-sm-6">
                            <div class="form-group form-check">
                                <input type="checkbox" name="modules[]" value="appointment"
                                    class="form-check-input module-permission" id="appointment"
                                    {{ in_array('appointment', (array) json_decode($role['module_access'])) ? 'checked' : '' }}>
                                <label class="form-check-label"
                                    style="{{ Session::get('direction') === 'rtl' ? 'margin-right: 1.25rem;' : '' }};"
                                    style="{{ Session::get('direction') === 'rtl' ? 'margin-right: 1.25rem;' : '' }};"
                                    for="appointment">{{ \App\CPU\translate('Appointment_Management') }}</label>
                            </div>
                        </div>
                        <div class="col-lg-3 col-sm-6">
                            <div class="form-group form-check">
