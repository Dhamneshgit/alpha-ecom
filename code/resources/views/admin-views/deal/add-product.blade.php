                    ...
                    <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100" cellspacing="0">
                        ...
                        @foreach($products as $k=>$de_p)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $f_product = \App\Model\FlashDealProduct::with('seller')->where('flash_deal_id', $deal['id'])->where('product_id',$de_p['id'])->first();
                                    @endphp
                                    @if($f_product->seller_type == 'seller')
                                    <a href="{{ route('admin.sellers.view', [$f_product->seller_id]) }}">
                                        {{ ucfirst($de_p->seller?->f_name) }} {{ ucfirst($de_p->seller?->l_name) }} <br> 
                                        {{ $de_p->seller?->unique_code }}
                                    </a>
                                    @else
                                ...
                                <td>
                                    @if($f_product->status == 0)
                                    <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#exampleModal{{$f_product->id}}">
                                        Pending
                                        <i class="tio-edit"></i>
                                    </button>
                                    <!-- <span class="badge badge-warning">Pending</span> -->
                                    @elseif($f_product->status == 1)
                                    <button type="button" class="btn btn-success" data-toggle="modal" data-target="#exampleModal{{$f_product->id}}">
                                        Approved
                                        <i class="tio-edit"></i>
                                    </button>
                                    <!-- <span class="badge badge-success">Approved</span> -->
                                    @else
                                    <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#exampleModal{{$f_product->id}}">
                                        Reject
                                        <i class="tio-edit"></i>
                                    </button>
                                    ...
                                    <div class="modal fade" id="exampleModal{{$f_product->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <form action="{{ route('admin.deal.update-product-status')}}" method="post">
                                                @csrf
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <input type="hidden" name="flash_id" value="{{$f_product->id}}">

                                                        <div class="form-group">
                                                            <div class="row">
                                                                <div class="col-md-12">
                                                                    <label for="name" class="title-color text-capitalize">{{ \App\CPU\translate('Status')}}</label>
                                                                    <select class="form-control" name="status">
                                                                        <option value="0" @if($f_product->status == 0 ) selected @endif> {{ \App\CPU\translate('Pending')}} </option>
                                                                        <option value="1" @if($f_product->status == 1 ) selected @endif> {{ \App\CPU\translate('Approve')}} </option>
                                                                        <option value="2" @if($f_product->status == 2 ) selected @endif> {{ \App\CPU\translate('Rejected')}} </option>
