...
@section('content')
    ...
    $direction = Session::get('direction') === "rtl" ? 'ml-3' : 'mr-3';
@endphp

<ul class="list-unstyled list-unstyled-py-2 mb-0">
    @foreach([5, 4, 3, 2, 1] as $rating)
        @php
            $count = \App\CPU\Helpers::rating_count($product['id'], $rating);
            $percentage = $total == 0 ? 0 : ($count / $total) * 100;
        @endphp
        <li class="d-flex align-items-center font-size-sm">
            <span class="{{ $direction }}">{{ \App\CPU\translate("{$rating} star") }}</span>
            <div class="progress flex-grow-1">
                <div class="progress-bar" role="progressbar" style="width: {{ $percentage }}%;" 
                     aria-valuenow="{{ $percentage }}" aria-valuemin="0" aria-valuemax="100">
                </div>
            </div>
            <span class="{{ $direction }}">{{ $count }}</span>
        </li>
    @endforeach
</ul>
                    </div>
    ...
    @if (count($colors) > 0)
        <div class="d-flex align-items-center flex-wrap gap-10 mb-4">
            <div class="">
                <div class="product-description-label">{{ \App\CPU\translate('Available color') }}:</div>
            </div>
            <div class="">
                <ul class="align-items-center d-flex flex-wrap gap-2 list-inline mb-0">
                    @foreach ($colors as $key => $color)
                        <li>
                            <label class="d-block p-2" style="background: {{ $color }};" for="{{ $product->id }}-color-{{ $key }}"></label>
                        </li>
                    @endforeach
                </ul>
            </div>
