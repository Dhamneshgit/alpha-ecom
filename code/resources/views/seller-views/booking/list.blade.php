...
@section('title', \App\CPU\translate('Booking List'))
        ...
        <div>
            ...
            <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
                <h2 class="h1 mb-0">
                    ...
                    {{\App\CPU\translate('Service Booking')}}
                </h2>
                ...
                <div class="card-body">
                        ...
                        <div class="col-sm-6 col-lg-3">
                            <div class="order-stats order-stats_delivered cursor-pointer">
                                <div class="order-stats__content" style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};">
                                    ...
                                    <h6 class="order-stats__subtitle">{{\App\CPU\translate('Completed')}}</h6>
                                </div>
                                <span class="order-stats__title">
                                    {{ $completed_count }}
                                </span>
                    ...
                    <div class="px-3 py-4 light-bg">
                        <div class="row g-2 flex-grow-1">
                            ...
                            <!-- <div class="col-sm-4 col-md-6 col-lg-8 d-flex justify-content-sm-end d-none">
                        ...
                        <table class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table w-100"
                            ...
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    ...
                                    <th>{{\App\CPU\translate('Booking')}} {{\App\CPU\translate('ID')}}</th>
                                    <th>{{\App\CPU\translate('Booking')}} {{\App\CPU\translate('Date')}}</th>
                                    <th>{{\App\CPU\translate('customer')}} {{\App\CPU\translate('info')}}</th>
                                    {{-- <th>{{\App\CPU\translate('Seller')}}</th> --}}
                                    <th>{{\App\CPU\translate('Sevice Name')}}</th>
                                    <th class="text-right">{{\App\CPU\translate('Booking')}} {{\App\CPU\translate('Amount')}}</th>
                            ...
                            @foreach($orders as $key=>$order)
                                ...
                                <tr class="status-{{$order['order_status']}} class-all">
                                    ...
                                    <td >
                                        <a class="title-color" href="{{route('admin.bookings.details',['id'=>$order['id']])}}">{{$order['booking_id']}}</a>
                                    </td>
                                    <td>
                                        <div>{{date('d M Y',strtotime($order['booking_datetime']))}},</div>
                                        <div>{{ date("h:i A",strtotime($order['booking_datetime'])) }}</div>
                                    </td>
                                    <td>
                                        @if($order->user_id == 0)
                                        ...
                                        @else
                                            @if($order->user)
                                                <a class="text-body text-capitalize" href="{{route('admin.orders.details',['id'=>$order['id']])}}">
                                                    <strong class="title-name">{{$order->user['f_name'].' '.$order->user['l_name']}}</strong>
                                                </a>
                                                <a class="d-block title-color" href="tel:{{ $order->user['phone'] }}">{{ $order->user['phone'] }}</a>
                                            @else
                                    ...
                                    {{-- <td>
                                        <span class="store-name font-weight-medium">
                                           {{ isset($order->seller) ? ucfirst($order->seller['f_name'].' '.$order->seller['l_name']) : 'not found' }}
                                        </span>
                                    </td> --}}
                                    <td>
                                        <span class="store-name font-weight-medium">
                                           {{ isset($order->service) ? ucfirst($order->service['name']) : 'not found' }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="text-right">
                                        <div>
                                            ...
                                            @endif
                                            {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->paid_amount+$discount))}}
                                        </div>
                                    ...
                                    <td class="text-center text-capitalize">
                                        ...
                                        @elseif($order['status']==2)
                                            <span class="badge badge-soft-primary fz-12">
                                                {{\App\CPU\translate('Completed')}}
                                            </span>
                                        @else
                                            <span class="badge badge-soft-warning fz-12">
                                                {{\App\CPU\translate('Re-Scheduled')}}
                                            </span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            <a class="btn btn-outline--primary square-btn btn-sm mr-1" title="{{\App\CPU\translate('view')}}"
                                                href="{{route('seller.bookings.details',['id'=>$order['id']])}}">
                                                <img src="{{asset('/public/assets/back-end/img/eye.svg')}}" class="svg" alt="">
                                            </a>
                                            <a class=" d-none btn btn-outline-success square-btn btn-sm mr-1" target="_blank" title="{{\App\CPU\translate('invoice')}}"
