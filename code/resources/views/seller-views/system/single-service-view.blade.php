    ...
    <div class="content container-fluid" style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLongTitle">Manage Time Slots</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form id="timeslotForm" action="{{ route('seller.service.storeTimeSlotes') }}" method="POST">
                        @csrf
                        <div class="modal-body">
                            <input type="hidden" name="service_id" value="{{ $product['id'] }}">
                            <table class="table" id="timeSlotTable">
                                <thead>
                                    <tr>
                                        <th>From Time</th>
                                        <th>To Time</th>
                                ...
                                <tbody>
                                    <tr id="row-1">
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-danger removeRowBtn" data-row-id="1">
                                                Remove
                                            </button>
                                        </td>
                                    </tr>
        ...
        @if ($product['added_by'] == 'seller' && ($product['request_status'] == 0 || $product['request_status'] == 1))
            <div class="d-flex justify-content-sm-end flex-wrap gap-2 mb-3">
                ...
                <div>
                    <button class="d-none btn btn-warning" data-toggle="modal" data-target="#publishNoteModal">
                    ...
                    <div class="col-md-auto mb-3 mb-md-0">
                        <div class="d-flex align-items-center">
                            <img class="avatar avatar-xxl avatar-4by3 {{ Session::get('direction') === 'rtl' ? 'ml-4' : 'mr-4' }}"
                                ...
                                src="{{ \App\CPU\ProductManager::product_image_path1('thumbnail') }}/{{ $product['thumbnail'] }}"
                    ...
                    <div class="col-md">
                        <ul class="list-unstyled list-unstyled-py-2 mb-0">
                            ...
                            <li class="d-flex align-items-center font-size-sm">
                                @php($five = \App\CPU\Helpers::service_rating_count($product['id'], 5))
                                <span
                            ...
                            <!-- Review Ratings -->
                            <li class="d-flex align-items-center font-size-sm">
                                @php($four = \App\CPU\Helpers::service_rating_count($product['id'], 4))
                                <span
                            ...
                            <!-- Review Ratings -->
                            <li class="d-flex align-items-center font-size-sm">
                                @php($three = \App\CPU\Helpers::service_rating_count($product['id'], 3))
                                <span
                            ...
                            <!-- Review Ratings -->
                            <li class="d-flex align-items-center font-size-sm">
                                @php($two = \App\CPU\Helpers::service_rating_count($product['id'], 2))
                                <span
                            ...
                            <!-- Review Ratings -->
                            <li class="d-flex align-items-center font-size-sm">
                                @php($one = \App\CPU\Helpers::service_rating_count($product['id'], 1))
                                <span
                ...
                <table
                    ...
                    <thead class="thead-light thead-50 text-capitalize">
                        <tr>
                            ...
                            <th>{{ \App\CPU\translate('From Time') }}</th>
                            <th>{{ \App\CPU\translate('To Time') }}</th>
                    ...
                    <tbody>
                        @php($i = 1)
                        @foreach ($timeslot as $review)
                            <tr>
                                <td>
                                    {{ $i }}
                                </td>
                                <td>
                                    {{ \Carbon\Carbon::parse($review['from_time'])->format('h:i A') }}
                                </td>
                                <td>
                                    {{ \Carbon\Carbon::parse($review['to_time'])->format('h:i A') }}
                                </td>
                                <td class="d-flex justify-content-center gap-2">
                                    ...
                                    <a class="btn btn-outline-danger btn-sm square-btn" href="javascript:"
                                        ...
                                        onclick="form_alert('product-{{ $review['id'] }}','are you Want to delete this timeslot ?')">
                                        <i class="tio-delete"></i>
                                    </a>

                                    <form action="{{ route('seller.service.slotdelete', [$review['id']]) }}" method="post"
                                    ...
                                    <div class="modal fade" id="editModal{{ $review['id'] }}" tabindex="-1"
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="editModalLabel">Update Time Slot</h5>
                                                    <button type="button" class="close" data-dismiss="modal"
                                                        aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                </div>
                                                <form action="{{ route('seller.service.updateTimeSlotes') }}"
                                                    method="POST">
                                                    @csrf
                                                    <div class="modal-body">
                                                        <table class="table">
                                                                ...
                                                                <tr>
                                                                    <td>
                                                                        ...
                                                                        <input type="hidden" name="service_id"
                                                                            class="form-control"
                                                                            value="{{ $review['service_id'] }}" required>
                                                                        <input type="time" name="from_time"
                                                                            class="form-control"
                                                                            value="{{ $review['from_time'] }}" required>
                                                                    </td>
                                                                    <td>
                                                                        <input type="time" name="to_time"
                                                                            class="form-control"
                                                                            value="{{ $review['to_time'] }}" required>
                                                                    </td>
...
@push('script_2')
    ...
    <script>
        $(document).ready(function () {
            let rowCount = 1; // Start with one default row
        ...
        $('#addRowBtn').click(function () {
            ...
            const newRow = `
                <tr id="row-${rowCount}">
                    ...
                    <td>
                        <button type="button" class="btn btn-danger removeRowBtn" data-row-id="${rowCount}">
                            Remove
                        </button>
                    </td>
                </tr>
            `;
            $('#timeSlotTable tbody').append(newRow);
        });

        // Remove a specific row
        $('#timeSlotTable').on('click', '.removeRowBtn', function () {
            const rowId = $(this).data('row-id');
            $(`#row-${rowId}`).remove();

            // If no rows are left, add a default row
            if ($('#timeSlotTable tbody tr').length === 0) {
                rowCount++;
                const defaultRow = `
                    <tr id="row-${rowCount}">
                        <td>
                ...
                `;
                $('#timeSlotTable tbody').append(defaultRow);
            }
        });
            ...
            //     // Example: 
            //     // $.post('/save-timeslots', formData, function(response) {
