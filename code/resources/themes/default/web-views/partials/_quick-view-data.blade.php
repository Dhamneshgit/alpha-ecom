                ...
                <form id="add-to-cart-form" class="mb-2">
                    ...
                    <div class="__btn-grp align-items-center mt-2">
                        @if(($product->added_by == 'seller' && ($seller_temporary_close || (isset($product->seller->shop) && $product->seller->shop->vacation_status && $current_date >= $seller_vacation_start_date && $current_date <= $seller_vacation_end_date))) ||
                            ...
                            <button class="btn buyNowBtn" type="button" disabled>
                                {{\App\CPU\translate('buy_now')}}
                            </button>
                            <button class="btn btn--primary addtoCartBtn string-limit" type="button" disabled>
                                {{\App\CPU\translate('add_to_cart')}}
                            </button>
                        @else
                            <button class="btn buyNowBtn" onclick="buy_now()" type="button">
                                {{\App\CPU\translate('buy_now')}}
                            </button>
                            <button class="btn btn--primary addtoCartBtn string-limit" onclick="addToCart()" type="button">
                                {{\App\CPU\translate('add_to_cart')}}
                            </button>
                        @endif
                        <button type="button" onclick="addWishlist('{{$product['id']}}')"
                                class="text-danger wishlistBtn btn string-limit">
