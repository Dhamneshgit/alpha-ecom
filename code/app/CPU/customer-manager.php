...
class CustomerManager
{
    ...
    public static function create_wallet_transaction1($user_id, float $amount, $transaction_type, $referance)
    {
        if(BusinessSetting::where('type','wallet_status')->first()->value != 1) return false;
        $user = User::find($user_id);
        $current_balance = $user->repurchase_wallet;
        ...
        $wallet_transaction->created_at = now();
        $wallet_transaction->updated_at = now();
        $user->repurchase_wallet = $current_balance + Convert::usd($credit) - Convert::usd($debit);
