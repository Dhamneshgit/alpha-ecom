...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        $seller_data1 = Seller::find($seller_data->seller_id);
        $seller_data12 = SellerWallet::find($seller_data->seller_id);


        if (!empty($seller_data1->sales_commission_percentage)) {
            $maintainBalance = DB::table('business_settings')->where('type', 'minimum_seller_wallet')->first();
            // if($maintainBalance->value < $seller_data1->wallet_maintain){
            if ($maintainBalance->value < $seller_data12->total_earning) {
                $order = Order::find($order_id);
                $sellerCommission = $seller_data1->sales_commission_percentage;
                $adminCommission = ($sellerCommission / 100) * $order->order_amount;

                $seller_data12->total_earning =  $seller_data12->total_earning - round($adminCommission, 2);
                $seller_data12->save();
            }
            ...
            //         $district->save();
            //         $d4 = Helpers::repurchaseTransactions($district->id, $user_id, $districtBonus, 'distirct', $userdata->zipcode, 'district_repurchase_bonus');
            //     }
            ...
            //         $state->save();
            //         $d5 = Helpers::repurchaseTransactions($state->id, $user_id, $stateBonus, 'state', $userdata->zipcode, 'state_repurchase_bonus');
            //     }
            ...
            //         $frenchise->save();
            //         $d3 = Helpers::repurchaseTransactions($frenchise->id, $user_id, $frenchiseBonus, 'frenchise', $userdata->zipcode, 'frenchise_repurchase_bonus');
            //     }
            ...
            //     $data = Helpers::get_bonus_referral($user_id, 1, $remainingAmount);
