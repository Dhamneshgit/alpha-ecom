...
class ProductManager
{
    ...
    public static function get_latest_products_by_zipcode($zipcode, $limit = 10, $offset = 1)
    {
        $paginator = Product::active()
                            ->with(['rating', 'tags', 'seller.shop'])
                            ->whereHas('seller', function($query) use ($zipcode) {
    ...
    public static function get_featured_products_by_zipcode($zipcode,$limit = 10, $offset = 1 )
    {
        $paginator = Product::with(['rating', 'tags','seller'])
            ->active()
            ->where('featured', 1)
            ->whereHas('seller', function($query) use ($zipcode) {
                $query->where('zipcode', $zipcode);
    ...
    public static function product_image_path1($image_type)
    {
        $path = '';
        if ($image_type == 'thumbnail') {
            $path = asset('storage/app/public/product');
