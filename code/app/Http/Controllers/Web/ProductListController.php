...
class ProductListController extends Controller
{
    ...
    public function default_theme($request){
        ...
        $porduct_data = Product::active()->with(['reviews', 'seller.shop']);

        if ($city) {
            $porduct_data = $porduct_data->whereHas('seller', function ($query) use ($city) {
                $query->where('city', $city);
            });
        }

        $maxPrice = $porduct_data->max('unit_price');
        ...
        return view(VIEW_FILE_NAMES['products_view_page'], compact('products', 'data', 'maxPrice' ));
    }
