...
class UserProfileController extends Controller
{
    ...
    //     return view('web-views.users-profile.my_services_view', compact('bookings'));
    // }
    ...
    public function my_service(Request $request)
    {
        ...
        // Step 1: Fetch the bookings for the user (without the service data)
        $bookings = Booking::where('user_id', $user_id)
            ->orderBy('created_at', 'desc')
            ->get();

        // Step 2: Fetch the service data for each unique service_id in bookings
        $service_ids = $bookings->pluck('service_id')->unique(); // Get unique service_ids from bookings
        $services = Service::whereIn('id', $service_ids)->get(); // Fetch services based on those ids

        // Step 3: Pass both bookings and services to the view
        return view('web-views.users-profile.my_services_view', compact('bookings', 'services'));
    }
    public function return_orders(Request $request)
    {
        // $return_orders_count = Order::where('order_status', 'returned')->count();
        // session(['return_orders_count' => $return_orders_count]);


        $return_orders = Order::where('order_status', 'Returned')
            ->where('customer_id', auth('customer')->id())
            ->get();

        return view('web-views.users-profile.return_order_view', compact('return_orders'));
    }
    ...
    public function return_order_details(Request $request)
    {
        $order = $this->order->with(['details.product', 'delivery_man_review'])->where(['customer_id' => auth('customer')->id()])->find($request->id);
        $refund_day_limit = \App\CPU\Helpers::get_business_settings('refund_day_limit');
        $current_date = \Carbon\Carbon::now();
        if ($order) {
            return view('web-views.users-profile.return-order-details', compact('order', 'refund_day_limit', 'current_date'));
        }

        Toastr::warning('Invalid order');
        return redirect('return-orders');
    }
    ...
    public function payment_option(Request $request)
    {
        ...
        if ($payment_status == 'paid') {
            ...
            $order_data->payment_method = $request->payment_method ?? 'razor_pay';

            $order_data->transaction_ref = $request->transaction_ref ?? null;

            $order_data->payment_status = 'paid';

            $order_data->save();

            return response()->json(['message' => 'Payment done', 'order' => $order_data], 200);
        }
    }
}
