...
class BookingController extends Controller
{
    ...
    public function list(Request $request, $status)
    {
        ...
        $orders = Order::with(['customer', 'seller.shop','assigned_by'])
            ->when($status != 'all', function ($q) use ($status) {
                $q->where(function ($query) use ($status) {
                    $query->orWhere('order_status', $status);
                });
            })
            ->when($manufacturer_id != '', function ($q) use ($manufacturer_id) {
                $q->where('manufacturer_id', $manufacturer_id);
            })
            ->when($aggregator_id != '', function ($q) use ($aggregator_id) {
                $q->where('aggregator_id', $aggregator_id);
            })
        ...
        // Add conditions based on manufacturer_id and aggregator_id
        if ($manufacturer_id != '') {
            $pending_count = $this->common_query_status_count(Order::where(['order_status' => 'pending', 'manufacturer_id' => $manufacturer_id]), $status, $request);
            $confirmed_count = $this->common_query_status_count(Order::where(['order_status' => 'confirmed', 'manufacturer_id' => $manufacturer_id]), $status, $request);
            $processing_count = $this->common_query_status_count(Order::where(['order_status' => 'processing', 'manufacturer_id' => $manufacturer_id]), $status, $request);
            $out_for_delivery_count = $this->common_query_status_count(Order::where(['order_status' => 'out_for_delivery', 'manufacturer_id' => $manufacturer_id]), $status, $request);
            $delivered_count = $this->common_query_status_count(Order::where(['order_status' => 'delivered', 'manufacturer_id' => $manufacturer_id]), $status, $request);
            $canceled_count = $this->common_query_status_count(Order::where(['order_status' => 'canceled', 'manufacturer_id' => $manufacturer_id]), $status, $request);
            $returned_count = $this->common_query_status_count(Order::where(['order_status' => 'returned', 'manufacturer_id' => $manufacturer_id]), $status, $request);
            $failed_count = $this->common_query_status_count(Order::where(['order_status' => 'failed', 'manufacturer_id' => $manufacturer_id]), $status, $request);
        } else if ($aggregator_id != '') {
            $pending_count = $this->common_query_status_count(Order::where(['order_status' => 'pending', 'aggregator_id' => $aggregator_id]), $status, $request);
            $confirmed_count = $this->common_query_status_count(Order::where(['order_status' => 'confirmed', 'aggregator_id' => $aggregator_id]), $status, $request);
            $processing_count = $this->common_query_status_count(Order::where(['order_status' => 'processing', 'aggregator_id' => $aggregator_id]), $status, $request);
            $out_for_delivery_count = $this->common_query_status_count(Order::where(['order_status' => 'out_for_delivery', 'aggregator_id' => $aggregator_id]), $status, $request);
            $delivered_count = $this->common_query_status_count(Order::where(['order_status' => 'delivered', 'aggregator_id' => $aggregator_id]), $status, $request);
            $canceled_count = $this->common_query_status_count(Order::where(['order_status' => 'canceled', 'aggregator_id' => $aggregator_id]), $status, $request);
            $returned_count = $this->common_query_status_count(Order::where(['order_status' => 'returned', 'aggregator_id' => $aggregator_id]), $status, $request);
            $failed_count = $this->common_query_status_count(Order::where(['order_status' => 'failed', 'aggregator_id' => $aggregator_id]), $status, $request);
        } else {
    ...
    public function serviceList(Request $request)
    {
        ...
        $orders = Booking::with(['seller', 'user','service'])
            ->when($request->has('search') && $search != null, function ($q) use ($key) {
                $q->where(function ($qq) use ($key) {
                    foreach ($key as $value) {
                        $qq->where('booking_id', 'like', "%{$value}%")
                            ->orWhere('patient_mobile', 'like', "%{$value}%")
        ...
        // Add conditions based on manufacturer_id and aggregator_id
            ...
            $pending_query = Booking::where(['status' => 0]);
            $pending_count = $this->common_query_status_count($pending_query, $status, $request);

            $confirmed_query = Booking::where(['status' => 1]);
            $confirmed_count = $this->common_query_status_count($confirmed_query, $status, $request);

            $completed_query = Booking::where(['status' => 2]);
            $completed_count = $this->common_query_status_count($completed_query, $status, $request);

            // $out_for_delivery_query = Booking::where(['status' => 3]);
            // $out_for_delivery_count = $this->common_query_status_count($out_for_delivery_query, $status, $request);

            $delivered_query = Booking::where(['status' => 3]);
            $delivered_count = $this->common_query_status_count($delivered_query, $status, $request);

            $canceled_query = Booking::where(['status' => 4]);
            $canceled_count = $this->common_query_status_count($canceled_query, $status, $request);
    ...
    public function doctorAppointments(Request $request)
    {
        ...
        $orders = Booking::with(['employee', 'user'])
            ->when($homevisit_id != '', function ($q) use ($homevisit_id) {
                $q->where('employee_id', $homevisit_id);
            })
            ->when($doctor_id != '', function ($q) use ($doctor_id) {
                $q->where('employee_id', $doctor_id);
            })
            ->when($request->has('search') && $search != null, function ($q) use ($key) {
                $q->where(function ($qq) use ($key) {
                    foreach ($key as $value) {
                        $qq->where('id', 'like', "%{$value}%")
                            ->orWhere('patient_name', 'like', "%{$value}%");
                    }
                });
            })->when(!empty($from) && !empty($to), function ($dateQuery) use ($from, $to) {
                $dateQuery->whereDate('created_at', '>=', $from)
        ...
        // Add conditions based on manufacturer_id and aggregator_id
        if ($homevisit_id != '') {
            $pending_count = $this->common_query_status_count(Booking::where(['status' => 0, 'employee_id' => $homevisit_id, 'booking_type' => 'home_visit']), $status,$request);
            $confirmed_count = $this->common_query_status_count(Booking::where(['status' => 1, 'employee_id' => $homevisit_id , 'booking_type' => 'home_visit']), $status, $request);
            $completed_count = $this->common_query_status_count(Booking::where(['status' => 2, 'employee_id' => $homevisit_id , 'booking_type' => 'home_visit']), $status, $request);
            $delivered_count = $this->common_query_status_count(Booking::where(['status' => 3, 'employee_id' => $homevisit_id , 'booking_type' => 'home_visit']), $status, $request);
            $canceled_count = $this->common_query_status_count(Booking::where(['status' => 4, 'employee_id' => $homevisit_id , 'booking_type' => 'home_visit']), $status, $request);
        } else if ($doctor_id != '') {
            $pending_count = $this->common_query_status_count(Booking::where(['status' => 0, 'employee_id' => $doctor_id , 'booking_type' => 'doctor']), $status, $request);
            $confirmed_count = $this->common_query_status_count(Booking::where(['status' => 1, 'employee_id' => $doctor_id , 'booking_type' => 'doctor']), $status, $request);
            $completed_count = $this->common_query_status_count(Booking::where(['status' => 2, 'employee_id' => $doctor_id , 'booking_type' => 'doctor']), $status, $request);
            $delivered_count = $this->common_query_status_count(Booking::where(['status' => 3, 'employee_id' => $doctor_id , 'booking_type' => 'doctor']), $status, $request);
            $canceled_count = $this->common_query_status_count(Booking::where(['status' => 4, 'employee_id' => $doctor_id , 'booking_type' => 'doctor']), $status, $request);
        }  else {
            $pending_query = Booking::where(['status' => 0])->where('booking_type','doctor');
            $pending_count = $this->common_query_status_count($pending_query, $status, $request);

            $confirmed_query = Booking::where(['status' => 1])->where('booking_type','doctor');
            $confirmed_count = $this->common_query_status_count($confirmed_query, $status, $request);

            $completed_query = Booking::where(['status' => 2])->where('booking_type','doctor');
            $completed_count = $this->common_query_status_count($completed_query, $status, $request);

            // $out_for_delivery_query = Booking::where(['status' => 3]);
            // $out_for_delivery_count = $this->common_query_status_count($out_for_delivery_query, $status, $request);

            $delivered_query = Booking::where(['status' => 3])->where('booking_type','doctor');
            $delivered_count = $this->common_query_status_count($delivered_query, $status, $request);

            $canceled_query = Booking::where(['status' => 4])->where('booking_type','doctor');
            $canceled_count = $this->common_query_status_count($canceled_query, $status, $request);
    ...
    public function homevisitAppointments(Request $request)
    {
        $status = 'all';
        $currentUserId = auth('admin')->id();
        ...
        if ($homevisit_id != '') {
            ...
            $pending_query = Booking::where(['status' => 0])->where('booking_type','home_visit');
            $pending_count = $this->common_query_status_count($pending_query, $status, $request);

            $confirmed_query = Booking::where(['status' => 1])->where('booking_type','home_visit');
            $confirmed_count = $this->common_query_status_count($confirmed_query, $status, $request);

            $completed_query = Booking::where(['status' => 2])->where('booking_type','home_visit');
            $completed_count = $this->common_query_status_count($completed_query, $status, $request);

            // $out_for_delivery_query = Booking::where(['status' => 3]);
            // $out_for_delivery_count = $this->common_query_status_count($out_for_delivery_query, $status, $request);

            $delivered_query = Booking::where(['status' => 3])->where('booking_type','home_visit');
            $delivered_count = $this->common_query_status_count($delivered_query, $status, $request);

            $canceled_query = Booking::where(['status' => 4])->where('booking_type','home_visit');
            $canceled_count = $this->common_query_status_count($canceled_query, $status, $request);
    ...
    public function details($id)
    {
        ...
        $manufacturer = DB::table('admins')
            ->where('admin_role_id', '2')
            ->where('status', '1')
            ->get();
        $aggregator = DB::table('admins')
    ...
    public function appointmentDetails($id)
    {
        ...
        $order = Booking::with('seller.shop','user')->where(['id' => $id])->first();
        ...
        ->where('status', '1')
        ->get();
        $doctor = DB::table('admins')
            ->where('admin_role_id', '5')
            ->where('status', '1')
            ->get();
        $homevisit = DB::table('admins')
            ->where('admin_role_id', '4')
            ->where('status', '1')
            ->get();

        $seller = Seller::where('type','service')
            ->where('status', 'approved')
            ->get();
            // dd($aggregator);
        // $shipping_address = ShippingAddress::find($order->shipping_address);
        // if ($order->order_type == 'default_type') {
            return view('admin-views.booking.order-details', compact(
                'manufacturer',
                'aggregator',
                'doctor',
                'homevisit',
    ...
    public function add_delivery_man($order_id, $delivery_man_id, $type)
    {
        // dd($type);
        ...
        if ($type == 'manufacturer') {
            ...
            $order->delivery_man_id = 1;
            $order->manufacturer_id = $delivery_man_id;
            $order->assign_by = auth('admin')->id() ?? 1;
            ...
            $order = Order::find($order_id);
            $order->delivery_man_id = 1;
            $order->aggregator_id = $delivery_man_id;
            $order->assign_by = auth('admin')->id() ?? 1;
            $order->delivery_type = null;
            $order->delivery_service_name = null;
            $order->third_party_delivery_tracking_id = null;
            $order->save();
        } else if ($type == 'm_aggregator') {
            $order = Order::find($order_id);
            $order->delivery_man_id = 1;
            $order->aggregator_id = $delivery_man_id;
            $order->assign_by = auth('admin')->id() ?? 1;
    ...
    public function assignBooking($order_id, $delivery_man_id, $type)
    {
        if ($delivery_man_id == 0) {
            return response()->json([], 401);
        }
        ...
        if ($type == 'doctor') {
            ...
            $order->seller_id = $delivery_man_id;
            $order->save();
    ...
    public function bookingStatus(Request $request)
    {
        ...
        if ($request->order_status == 2  && empty($order->employee_id)) {

            return response()->json(['payment_status' => 0], 200);
        }
        if ($order->status == 2) {

            return response()->json(['complete_status' => 0], 200);
        }

        $order->status = $request->order_status;
        $order->status_update_by = $user_id;
        $order->save();
    ...
    public function payment_status(Request $request)
    {
        if ($request->ajax()) {
            ...
            // $order = Order::find($request->id);
            $order->is_paid = $request->payment_status;
            $order->save();
