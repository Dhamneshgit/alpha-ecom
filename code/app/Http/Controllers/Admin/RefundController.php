...
class RefundController extends Controller
{
    ...
    public function refund_status_update(Request $request)
    {
        ...
        if($request->refund_status == 'refunded' && $refund->status != 'refunded')
        {
            ...
            if($order->seller_is == 'admin')
            {
                ...
                $seller_wallet->total_earning = $seller_wallet->total_earning - ($refund->amount - $order->admin_commission);
                $seller_wallet->refund_amount += $refund->amount;
                $seller_wallet->save();
    ...
    public function sellerList(Request $request)
    {
        ...
        return view('admin-views.refund-transaction.sellerlist', compact('refund_transactions', 'search', 'payment_method'));
    }
    ...
    //     $refund_transaction = RefundTransaction::with(['seller.shop'])
    //         ->selectRaw('payer_id, COUNT(id) as total_refunds, SUM(amount) as total_amount')
    ...
    public function refund_transaction_common_query($request)
    {
        ...
        $refund_transaction = RefundTransaction::join('sellers', 'refund_transactions.payer_id', '=', 'sellers.id')
            ->leftJoin('shops', 'sellers.id', '=', 'shops.seller_id') // Join shop details
            ->selectRaw('
                refund_transactions.payer_id, 
                sellers.f_name, 
                sellers.l_name, 
                sellers.unique_code, 
                shops.name as shop_name, 
                COUNT(refund_transactions.id) as total_refunds, 
                SUM(refund_transactions.amount) as total_amount
            ')
            ->when($search, function ($query) use ($search) {
                $key = explode(' ', $search);
                foreach ($key as $value) {
                    $query->orWhere('refund_transactions.order_id', 'like', "%{$value}%")
                        ->orWhere('refund_transactions.refund_id', 'like', "%{$value}%");
                }
            })
            ->when(!empty($payment_method) && $payment_method != 'all', function ($query) use ($payment_method) {
                $query->where('refund_transactions.payment_method', $payment_method);
            })
            ->where('refund_transactions.paid_by', 'seller')
            ->groupBy('refund_transactions.payer_id', 'sellers.f_name', 'sellers.l_name', 'sellers.unique_code', 'shops.name')
            ->latest('refund_transactions.created_at');
