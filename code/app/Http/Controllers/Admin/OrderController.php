    ...
    public function status(Request $request)
    {
        ...
        try {
            if (!empty($fcm_token)) {
                ...
                $token = $fcm_token;
                $title = 'Order Update';
                $body = 'Now Your Order Status is ' . ucfirst($request->order_status);

                $data = []; //$friendData;
                Helpers::sendNotification($token, $title, $body, $data);
                Helpers::createNotification($title, $body, $order->customer->user_id, null, null);
            }
        } catch (\Exception $e) {
        }
        ...
        try {
            ...
            if ($fcm_token_delivery_man) {
                ...
                $token = $fcm_token_delivery_man;
                $title = 'Order Update';
                $body = 'Order Id: ' . $order->id . ' Status is ' . ucfirst($request->order_status);

                $data = []; //$friendData;
                Helpers::sendNotification($token, $title, $body, $data);
                Helpers::createNotification($title, $body, null, null, $order->delivery_man->id);
            }
        } catch (\Exception $e) {
        }
