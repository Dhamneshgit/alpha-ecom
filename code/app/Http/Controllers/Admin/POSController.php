    ...
    public function place_order(Request $request)
    {
        ...
        if($zipcode->city_id){
            ...
            if($district){
                ...
                $district->save();
                $d4 = Helpers::zipcodeReferralTransaction($district->id, $user_id, $plan_data->district_bonus, 'district', $userdata->zipcode);
            ...
            if($state){
                ...
                $state->save();
                $d3 = Helpers::zipcodeReferralTransaction($state->id, $user_id, $plan_data->state_bonus, 'state', $userdata->zipcode);
            }
        }
        if($shopData){
            $shopData->earning_wallet = $shopData->earning_wallet + $plan_data->shop_bonus ;
            $shopData->save();
            $d1 = Helpers::zipcodeReferralTransaction($shopData->id, $user_id, $plan_data->shop_bonus, 'seller', $userdata->zipcode);
        }
        
        if($frenchise){
            $frenchise->user_bonus = $frenchise->user_bonus + $plan_data->frenchise_bonus;
            $frenchise->save();
            $d2 = Helpers::zipcodeReferralTransaction($frenchise->id, $user_id, $plan_data->frenchise_bonus, 'frenchise', $userdata->zipcode);
        }

        $is_frenchise = $userdata->is_frenchise;

        Helpers::get_bonus_referral($is_frenchise, $userdata->id);
