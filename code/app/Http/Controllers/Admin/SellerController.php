...
class SellerController extends Controller
{
    ...
    public function both_index(Request $request)
    {
        $query_param = [];
        $search = $request['search'];
        $current_date = date('Y-m-d');
        ...
        $admin = Admin::find(auth('admin')->id());
        // $zipcode = Zipcode::where('zipcode',$admin)find(auth('admin')->id());
    ...
    {
        $seller = Seller::with('sellerwallet')->find($id);
        if(!isset($seller))
        {
    ...
    public function fundRequest(){
        ...
        $data = DB::table('seller_fund_payments')
                    ->select('sellers.f_name','sellers.l_name','seller_fund_payments.*')
                    ->leftJoin('sellers','seller_fund_payments.user_id','=','sellers.id')
                    ->orderBy('id','DESC')
                    ->latest()
                    ->paginate(Helpers::pagination_limit());


        return view('admin-views.plan.seller_fund_list', compact('data'));
    }
    ...
    public function updatefundRequest(Request $request){
        ...
        DB::table('seller_fund_payments')->where('id',$request['id'])->update($update);
        $data = DB::table('seller_fund_payments')->where('id',$request['id'])->first();

        if($request['status'] == 1){
            $sellerwallet = SellerWallet::where('seller_id',$data->user_id)->first();
            $sellerwallet->total_earning = $sellerwallet->total_earning + $data->amount;
            $sellerwallet->save();

            Helpers::sellerWalletTransaction($data->user_id, $data->transaction_id, 'Credit', $data->amount, "Add Money");
    ...
    public function sellerCertificate($seller_id)
    {
        $seller = Seller::findOrFail($seller_id);;
        $shop = Shop::where('seller_id',$seller_id)->first();
        return view('admin-views.seller.certificate', compact('seller','shop'));
    }
    ...
    public function updateWallet(Request $request)
    {
        ...
        if (($request['type'] == 'debit') && ($request['amount'] >= $request['wallet_amount'])) {
            Toastr::error('Update amount is grater then the current amount, amount will not deduct');
            return back();
        }

        $sellerwallet = SellerWallet::where('seller_id',$request['seller_id'])->first();

        if ($request['type'] == 'debit'){
            $sellerwallet->total_earning -= $request['amount'];
        } else {
            $sellerwallet->total_earning += $request['amount'];
        }
        ...
        $type = ($request['type'] == 'debit') ? 'Debit' : 'Credit';
        ...
        $insert = [
            ...
            'status' =>1,
            'remark' => 'Admin '.$type.' '.$request['amount'].'Rs in your wallet', 
            'created_at' => now()
        ];
        DB::table('wallet_seller_transactions')->insert($insert);
        
        Toastr::success('Update amount successfully');
        return back();
    }
    ...
    {
        $zipcode = Zipcode::get();
        return view('admin-views.seller.add-new-seller',compact('zipcode'));
    }
}
