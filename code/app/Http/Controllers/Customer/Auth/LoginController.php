...
class LoginController extends Controller
{
    ...
    public function submit_with_otp(Request $request)
    {
        $request->validate([
    ...
    {
        $request->validate([
            'phone' => 'required|regex:/^[0-9]{10}$/', // Validate phone number
        ]);
        ...
        if (!$user) {
            Toastr::error(translate('User not found.'));
            return back()->withErrors(['phone' => 'User not registered.']);
        }
    ...
    public function logout(Request $request)
    {
        ...
        Session::forget('return_orders_count');
