...
class SellerController extends Controller
{
    ...
    public function getDeliveryMan(Request $request)
    {
        ...
        if ($seller->city) {
            $delivery_men = $delivery_men->where('city', $seller->city);
        }

        $delivery_men = $delivery_men->get();

        $response['status'] = true;
        $response['message'] = 'success';
        $response['data'] = $delivery_men;
    ...
    public function assignDeliveryMan(Request $request)
    {
        ...
        if(($order->order_status == 'pending') || ($order->order_status == 'confirmed') || ($order->order_status == 'processing')){
            ...
            $response['message'] = 'Order is Already '.$order->order_status;
            return response()->json($response);
        }
    ...
    public function seller_info(Request $request)
    {
        ...
        $data['minimum_wallet_maintain'] = $maintainBalance->value;
        $data['certificate'] = url('/seller_certificate/'.$seller['id']) ;
    ...
    public function getWalletTransaction(Request $request)
    {
        ...
        $data = DB::table('wallet_seller_transactions')->where('user_id',$seller['id'])->orderBy('id','DESC')->get();
        if($data){
            return response()->json([
                'status'=> true,
                'message'=> 'Sucessfully',
    ...
    public function addMoney(Request $request)
    {
        $seller = $request->seller;
        // dd($request);
        $sellerwallet = SellerWallet::where('seller_id',$seller['id'])->first();
        if($sellerwallet){
            $sellerwallet->total_earning = $sellerwallet->total_earning + $request['amount'];
            $sellerwallet->save();

            Helpers::sellerWalletTransaction($seller['id'], $request['transaction_id'], 'Credit', $request['amount'], "Add Money");
    ...
    public function requestMoney(Request $request)
    {
        ...
        if($sellerwallet){
            ...
            $check = DB::table('seller_fund_payments')->insert($data);
            if($check){
                return response()->json([
                    'status'=> true,
    ...
    public function getRequestMoney(Request $request)
    {
        $seller = $request->seller;
        if($seller){
            $check = DB::table('seller_fund_payments')->where('user_id',$seller['id'])->orderBy('id','DESC')->get();
            if($check){
                return response()->json([
                    'status'=> true,
                    'message'=>'Success',
    ...
    public function shop_info_update(Request $request)
    {
        ...
        if ($image != null) {
            $bottom_banner = ImageManager::upload('shop/banner/', 'png', $request->file('bottom_banner'));
        } else {
    ...
    public function bonustransaction(Request $request)
    {
       
        $seller = $request->seller;

        $transaction = RepurchaseTransaction::leftJoin('users as u2', 'u2.id', '=', 'repurchase_transactions.referral_id')
        ->where('repurchase_transactions.parent_type','=','shop')
        ->where('repurchase_transactions.parent_id','=',$seller['id'])
