...
class OrderController extends Controller
{
    ...
    public function order_detail_status(Request $request)
    {
        ...
        if ($request->order_status == 'delivered' && $order['seller_id'] != null) {
            ...
            $seller_data = Seller::find($order['seller_id']);
            ...
            if((!empty($userdata->plan_id)) && ($userdata->plan_expire_date > date('Y-m-d'))){
                ...
                if($district){
                    ...
                    $district->save();
                    $d4 = Helpers::repurchaseTransactions($district->id, $order['customer_id'], $districtBonus, 'distirct', $userdata->zipcode, 'district_repurchase_bonus');
                ...
                if($state){
                    ...
                    $state->save();
                    $d5 = Helpers::repurchaseTransactions($state->id, $order['customer_id'], $stateBonus, 'state', $userdata->zipcode, 'state_repurchase_bonus');
                }
            ...
            Helpers::sellerWalletTransaction($order['seller_id'], 'admin_commission', 'Debit', $adminCommission, "Admin Commissiom on Order ID:".$order['id']);
        }
