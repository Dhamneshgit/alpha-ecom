...
class DeliveryManController extends Controller
{
    ...
    public function update_order_status(Request $request)
    {
        ...
        try {
            ...
            if ($value && !empty($fcm_token)) {
                ...
                $token = $fcm_token; 
                $title = 'Order Update';
                $body = 'Order Id: '.$order->id.' Status is '.ucfirst($request['status']);
    ...
    public function order_status(Request $request)
    {
        ...
        $order = Order::where('delivery_man_id', $delivery_man->id)
                        ->where('id',$request->order_id)
                        ->first();
        if($order){
            $order->delivery_man_status = $request->status;
            $order->save();

            $status = ($request->status == 1) ? 'Accept' : 'Reject';
