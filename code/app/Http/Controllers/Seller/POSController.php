...
class POSController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        ...
        $seller_wallet = SellerWallet::find(auth('seller')->id());

        if(empty($seller_data->sales_commission_percentage)){
            Toastr::error(\App\CPU\translate('you_can_not_create_order_without_admin_set_your_commission'));
            return back();
        }
        ...
        if($maintainBalance->value > $seller_wallet->total_earning){
            Toastr::error(\App\CPU\translate('you_have_short_balance_for_wallet_maintain_amount'));
            return back();
        }
        $sellerCommission = $seller_data->sales_commission_percentage;
        $adminCommission = ($sellerCommission/100) * $request->amount;
        ...
        Helpers::sellerWalletTransaction(auth('seller')->id(), 'admin_commission', 'Debit', $adminCommission, "Admin Commissiom on Order ID:".$order_id);
