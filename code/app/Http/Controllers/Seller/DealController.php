...
class DealController extends Controller
{
    ...
    public function add_product($deal_id)
    {
        ...
        $flash_deal_products = FlashDealProduct::where('seller_id', $seller_id)->where('flash_deal_id', $deal_id)->pluck('product_id');
        ...
        $seller_product = Product::where('added_by','seller')->where('user_id',$seller_id)->active()->orderBy('name', 'asc')->get();

        return view('seller-views.deal.add-product', compact('deal', 'products','flash_deal_products','seller_product'));
    }
    ...
    public function add_product_submit(Request $request, $deal_id)
    {
        ...
        if(!isset($flash_deal_products))
        {
            ...
            Toastr::success('Product added successfully, Waiting for admin approval!');
            return back();
        }else{
